...
class PhotoLoss(nn.Module):
    ...
    def forward(self, outputs, side, mask=None):
        ...
        # Masked loss: only compute on valid regions
        if mask is not None:
            mask_tensor = outputs[mask.format(side)].bool()
            pred_valid = pred[mask_tensor]
            target_valid = target[mask_tensor]
            loss = 0
            if self.l1_rate != 0:
                loss += self.l1_rate * (pred_valid - target_valid).abs().mean()
            if self.l2_rate != 0:
                loss += self.l2_rate * ((pred_valid - target_valid)**2).mean()
            if self.ssim_rate != 0:
                raise NotImplementedError("SSIM not supported with mask")
            return loss

        # Original loss map computation
