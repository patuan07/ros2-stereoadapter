...
#from models.decoders.vmamba_decoder import VMambaDecoder
...
class TiO_Depth(Base_of_Model):
    def _initialize_model(
        ...
        if "GRU" in decoder_name:
            self.net_module['gru'] = GRUDecoder(corr_implementation, corr_radius, corr_levels, n_gru_layers, context_dims)
        #elif "VMamba" in decoder_name:
            #self.net_module['gru'] = VMambaDecoder(corr_implementation, corr_radius, corr_levels, n_gru_layers, context_dims)
    ...
    def forward(self, x, outputs, **kargs):
        ...
        if used_path == 'Mono':
            ...
            if hasattr(self, 'decoder_name') and ('GRU' in self.decoder_name or 'VMamba' in self.decoder_name):
                fmap2, fmap3 = outputs['enc_feats_s_wog'], outputs['enc_feats_o_wog']
                fmap2 = fmap2[1]
                fmap3 = fmap3[1]
                img2, img3 = x[:, :3, ...], x[:, 3:, ...]
                # TODO
                outputs = self._forward_gru_decoder_stereo(
                    outputs=outputs,
                    img2=img2, img3=img3,
                    fmap2=fmap2, fmap3=fmap3,
                    directs=directs,
                    iters=12,
                    use_scaled_mde_init=True  # 从 MDE 初始化 flow
                )
                #pred = outputs['stereo_depth_0_s']
                #outputs['sdepth'] = pred
                #pred_disp = outputs['flow_up']
    ...
    def _forward_gru_decoder_stereo(
        self,
        outputs,
        img2, img3,
        fmap2, fmap3,
        ...
        # self.cnet = self.net_module['encoder']
        cnet_list = self.net_module['encoder'](img2)[-3:]
        #print(cnet_list.shape)
        #print([x.shape for x in cnet_list])
        #print(fmap2.shape)
        outputs = self.net_module['gru'](self.d2d,
                                         outputs,
                                         img2, img3,
                                         fmap2, fmap3,
                                         directs,
