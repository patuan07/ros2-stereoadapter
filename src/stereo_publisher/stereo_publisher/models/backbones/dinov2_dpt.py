...
def get_dinov2_dpt_backbone(backbone_name='dinov2b', pretrained=True):
    ...
    if pth_path and os.path.exists(pth_path):
        ...
        print(f"Loaded pretrained backbone from {pth_path}")
    else:
        print(f"WARNING: Skipping pretrained backbone (pth_path={pth_path}). OK for inference if loading from full checkpoint.")
