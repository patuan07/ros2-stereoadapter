...
class ModelSaver(object):
    ...
    @_parallel_mask
    def save_model(self, model, optimizers, epoch, step, is_best, name='', extra_state=None):
        if self.is_parallel:
        ...
        save_dict['step'] = step
        for group_name, (optimizer, scheduler, _) in optimizers.items():
            save_dict[('optim_params', group_name)] = optimizer.state_dict()
            save_dict[('sched_params', group_name)] = scheduler.state_dict()

        # Save extra training state (e.g., lora_frozen, current_step)
    ...
    def load_model(self,
                   ...
                   model):
        map_location = torch.device('cpu')
        load_dict = torch.load(pre_model_path, map_location, weights_only=False)
        pretrained_dict = load_dict['model_params']
        model_dict = model._networks.state_dict()
    ...
    def load_optim(self, pre_model_path, optimizers=None):
        ...
        # 加载 extra_state（如果存在）
        extra_state = load_dict.get('extra_state', None)
        ...
        for group_name, (optimizer, scheduler, st_epoch) in optimizers.items():
            ...
            if ('sched_params', group_name) in load_dict:
                scheduler.load_state_dict(load_dict[('sched_params', group_name)])
            else:
                temp_epoch = 1
        ...
        return optimizers, start_epoch, start_step, extra_state
